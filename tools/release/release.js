...
const REPO = 'orangecoding/nanosearch'
...
async function createRelease() {
  try {
    console.log(c.cyan('🚀 Starting release process...'))

    // 1. Load Configuration
    if (!fs.existsSync(CONFIG_PATH)) {
      console.error(c.red('❌ Error: config.json not found in tools/release/'))
      process.exit(1)
    }

    if (!GITHUB_TOKEN) {
      console.error(c.red('❌ Error: GitHub token not configured.'))
      process.exit(1)
    }
    ...
    console.log(c.blue(`📦 Target version: ${version}`))

    // 3. Check if release already exists
    console.log(c.yellow('🔍 Checking if release already exists...'))
    ...
    if (existingReleaseResponse.status === 200) {
      console.error(c.red(`❌ Error: A release with tag ${tag} already exists.`))
      process.exit(1)
    }

    // 4. Fetch latest release to find the starting point for the diff
    console.log(c.yellow('📡 Fetching latest release from GitHub...'))
    ...
    if (!latestReleaseResponse.ok) {
      console.error(c.red('❌ Error fetching latest release.'))
      const errorData = await latestReleaseResponse.json()
      console.error(c.red(JSON.stringify(errorData)))
      process.exit(1)
    }

    const latestRelease = await latestReleaseResponse.json()
    const latestTag = latestRelease.tag_name
    console.log(c.green(`✅ Latest release found: ${latestTag}`))

    // 5. Ensure the latest tag is available locally
    console.log(c.yellow(`📡 Fetching tag ${latestTag} from remote...`))
    try {
      execSync(`git fetch origin tag ${latestTag} --no-tags`)
    } catch (error) {
      console.error(c.red(`❌ Error fetching tag ${latestTag} from origin.`))
    ...
    console.log(c.yellow(`Git diff: ${latestTag} .. HEAD`))
    let commitMessages
    try {
      commitMessages = execSync(`git log ${latestTag}..HEAD --pretty=format:"- %s"`).toString().trim()
    } catch (error) {
      console.error(c.red('❌ Error running git log. Make sure the latest tag is available locally.'), error)
      process.exit(1)
    }

    if (!commitMessages) {
      console.log(c.magenta('⚠️  No new commits found since last release.'))
    ...
    console.log(c.blue('📝 Opening editor for release notes (using nano or $EDITOR)...'))
    ...
    if (!editedContent) {
      console.error(c.red('❌ Release notes are empty. Aborting release.'))
      process.exit(1)
    }

    // 9. Create the new release
    console.log(c.cyan(`🚀 Creating release ${version} on GitHub...`))
    ...
    if (createResponse.status === 201) {
      ...
      console.log(c.green('🎉 Release successfully created!'))
      console.log(c.green(`🔗 URL: ${data.html_url}`))
    } else {
      const errorData = await createResponse.json()
      console.error(c.red('❌ Failed to create release.'))
      console.error(c.red(JSON.stringify(errorData, null, 2)))
      process.exit(1)
    }
  } catch (error) {
