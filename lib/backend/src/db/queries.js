...
/**
 * Returns a Map of path → { mtime, size } for every indexed file.
 * Used by the pipeline to detect new/changed files in a single query
 * instead of one query per file.
 * @param {import('better-sqlite3').Database} db
 * @returns {Map<string, { mtime: number, size: number }>}
 */
